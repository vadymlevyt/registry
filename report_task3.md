...
- Дії з ІНШИМИ існуючими справами — Дашборд або QI (обидва діють поза контекстом конкретного досьє)
- **СТВОРЕННЯ нової справи — тільки QI або кнопка «+ Додати справу» на головному реєстрі. Дашборд НЕ створює нові справи** (виправлено в micro-fix нижче)
...
4. **Створення нової справи прибрано з арсеналу агента досьє.** Бо це не дія в межах поточної — це створення сутності «поряд». Якщо адвокат скаже агенту досьє «створи нову справу» — агент має пояснити, що це робиться через **Quick Input** (команда на кшталт «створи справу для Іваненка») або через кнопку «+ Додати справу» на головному реєстрі. Дашборд **не створює** нові справи (див. micro-fix нижче — у первинному патчі тут була неточність).
...
- Попроси «створи нову справу для Іваненка» — агент має сказати, що це робиться через Quick Input або кнопку «+ Додати справу» на головному реєстрі (НЕ через Дашборд — Дашборд не створює нові справи).
...
# TASK 3 PATCH micro-fix — correct case-creation guidance

**Дата:** 2026-05-08 (того ж дня що і TASK 3 PATCH).
**Тригер:** адвокат зауважив фактологічну помилку — у системному промпті і в поясненні я писав, що для створення нової справи агент досьє направляє до «Quick Input або Дашборду». Дашборд **не створює** нові справи — він має лише `add_hearing/update_hearing/delete_hearing/add_note/update_note/delete_note/confirm_event/add_travel/batch_update`.

## Перевірка фактології (мало бути зроблено перед PATCH)

```
PERMISSIONS.qi_agent       — містить 'create_case'  ✓
PERMISSIONS.dashboard_agent — НЕ містить 'create_case'  ✗ (плутанина у попередній версії)
PERMISSIONS.dossier_agent   — містить 'create_case', але виключено з DOSSIER_AGENT_TOOLS у PATCH
```

**Точки створення нової справи в системі:**
1. **Quick Input** (агент, команда на кшталт «створи справу для Іваненка з категорії спадщина»).
2. **Кнопка «+ Додати справу»** на головному реєстрі (без агента, UI-форма).

Дашборд **редагує існуючі справи** — він додає засідання, нотатки, перетягує події у календарі — але не створює сутність "нова справа".

## Виправлення

**Системний промпт `CaseDossier/index.jsx` секція "## ОБМЕЖЕННЯ"** — переписано блок про напрямки. Тепер чітко:

- Дії з ІНШИМИ існуючими справами → Дашборд або QI (обидва діють поза контекстом конкретного досьє).
- Створення НОВОЇ справи → тільки QI або кнопка «+ Додати справу» на головному реєстрі. Явно сказано: «Дашборд НЕ створює нові справи — він тільки редагує існуючі».

## Висновок для майбутніх патчів

Перед формуванням системного промпту або пояснення адвокату — перевіряти фактологію через grep. Якщо стверджую що агент X щось вміє — спочатку звіряти з PERMISSIONS.X у App.jsx. Адвокат справедливо зауважив; цей принцип фіксую тут на майбутнє.

## Результат

- Зміни тільки в системному промпті агента досьє і в цьому звіті.
- Тести лишаються зеленими (текстова правка промпту не впливає на JSON Schema або runner).
- Білд чистий (промпт — рядкова конкатенація у buildAgentSystemPrompt).
