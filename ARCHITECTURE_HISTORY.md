...
## DP layout-leak + speed fixes (Phase B) — P1 / P2 / P3 / P4 (2026-05-20)

**Мета:** після Фази A (B1 layout-strip — ×48 економія розміру, 5-6 хв на
Брановського) — додаткова I/O оптимізація для майбутніх 200-250 стор.
томів з очисткою/стисненням. Кожен P-фікс ортогональний, кожен — окремий
коміт, кожен з власним тестом.

### Зроблено

- **P1 — паралелізація PERSIST Drive-uploads** (`be5c944`):
  `splitDocumentsV3.js` plan-loop / fallback-loop / fragments-uploads —
  for-of-await → `runWithConcurrency(items, fn, 5)`. Phase 1: CPU-prep
  (`buildDocumentPdf`, dedup) серіально; Phase 2: upload+persist+
  writeArtifacts паралельно з `PERSIST_CONCURRENCY=5`. Дедуп розширено:
  `registryView()` ∪ `pendingInBatch` (інакше дві однакові назви плану
  обидві пройдуть до уплоадів, бо `newDocuments` росте лише після persist).
  Контракт помилок збережено: UPLOAD_FAILED → file_skipped,
  PERSIST_FAILED → fatal. Новий загальний хелпер:
  `src/services/concurrency.js#runWithConcurrency` (один сенс: N тасків,
  M одночасно у польоті, результати у вхідному порядку).
- **P2 — trailing-debounce 800мс registry-save useEffect** (`5280fcd`):
  `App.jsx` EFFECT-B (auto-save до localStorage/Drive) переведено на
  `createDebouncedSave(saveFn, 800)`. Один сенс (правило #11): окрема
  гілка immediate-save для критичних дій (`destroy_case` у UI,
  `close_case`/`restore_case` в actionsRegistry через новий dep
  `requestImmediateSave`). Окремий `useEffect` навішує
  `visibilitychange→flush` і `unmount→flush` (захист від втрати останніх
  800мс при закритті вкладки). Новий примітив:
  `src/services/debouncedSave.js#createDebouncedSave`.
- **P3 — explicit timeout 60с Drive API через AbortController**
  (`fa3839e`): `driveAuth.driveRequest` створює внутрішній
  `AbortController` з `DRIVE_TIMEOUT_MS=60_000`. Caller може override через
  `options.timeoutMs` або `options.signal` (зовнішній signal → НЕ
  нав'язуємо власний timeout, life-cycle caller'а). `buildTimeoutSignal`
  повертає `{signal, clear}` — `clear` cleanup'ить таймер після
  завершення fetch (успіх/throw). Через `driveRequest` йдуть ВСІ ~50
  Drive-викликів системи (uploadBytes/readBytes/writeRegistry/...).
- **P4 — throttle jobState save до ≤1/10с з immediate-on-critical**
  (`eca8f9e`): `jobState.js` — додано `saveStateThrottled` (leading-edge:
  перший immediate, наступні у вікні `JOBSTATE_THROTTLE_MS=10_000` →
  pending trailing з НАЙСВІЖІШИМ станом). `flushPendingSave` —
  викликати перед terminal saveState. `saveState` (immediate) і
  `clearState` скасовують pending (leak-fix). Critical шляхи в
  `streamingExecutor` — immediate (upload_original failed, pipeline
  stopped, executor threw, cancelled). Non-critical (per-chunk OCR
  done, register chunks, file status change) — throttled. Per-chunk
  save був ~120/DP-run у Брановського.

### Тести / інваріанти

**1423 / 1423 зелені** (було 1392, +31 нових: 7 unit concurrency +
2 integration P1 + 8 unit debouncedSave + 6 unit driveAuth-timeout +
8 unit jobStateThrottle). Кожен фікс має тест (правило батьківського
TASK §2.1). Triage / OCR / classify / extract / моделі / паспорт /
диригент / схема / `DEFAULT_STAGE_ORDER` / `schemaVersion` — НЕ змінені.

### Очікуваний виграш (на планшеті, для Брановського-розміру)

| Фікс | Зменшення | Механізм |
|---|---|---|
| P1 | PERSIST з ~300-400 сек → ~60-100 сек (×3-5) | `Promise.all` з concurrency 5, ~6 HTTP per host |
| P2 | ~150 fires → ~3-5 saves (~10-15 сек) | trailing-debounce 800мс |
| P3 | прибирає невидимі багатогодинні зависання | explicit timeout 60с AbortController |
| P4 | ~120 saves → ~5-10 saves (~10-15 сек) | leading-edge throttle 10с + immediate-on-critical |

**Сумарно (порядок):** для Брановського 65 стор. → 25 нарізаних
очікувано **5-6 хв → 2-3 хв** end-to-end на планшеті. Для майбутніх
200-250 стор. з очисткою тексту і стисненням мультиплікативно — кожна
важка операція бачить уже паралельний/дебаунсений I/O.

### Поза скоупом

P1-P4 — мундейн I/O оптимізації, нуль зміни логіки/моделей/схеми/
диригента. Якість нарізки і поведінка стадій лишилась байт-у-байт така
сама (всі попередні тести зелені). Дослідження якості після цих фіксів —
окрема діагностика на реальній великій справі (тригер для D1 збагачений
паспорт — у tracking_debt #19).
