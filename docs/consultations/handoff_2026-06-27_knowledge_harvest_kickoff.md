...
## 0. ЗАВДАННЯ ОДНИМ ПОГЛЯДОМ (читай перед усім)

**Мета:** зібрати ВСЕ накопичене знання (репо-доки + zip-архіви чатів) → **один форвард майстер-план серверного переходу**, а принципи/ідеї розкласти по живих доках. Нічого цінного не втратити; сире — прибрати.

**Кроки:**
1. Прочитай цей файл + `docs/KNOWLEDGE_MAP.md` (карта: де що лежить за темами).
2. **Репо-трек** — за §2: HARVEST читай детально, PARTIAL — лише названий принцип, CANONICAL/SKIP не чіпай.
3. **Зіп-трек** (окремо, §3) — розпакуй `_harvest_inbox/` у scratch (процедура в README), триаж за рубрикою §1 з **продуктовими категоріями і RETAIN**.
4. Кожне зерно → у свій **дім** (§1, 3 шари): принцип → `DEVELOPMENT_PHILOSOPHY.md`; ідея/задача → `tracking_debt.md` (з тригером); крок → **майстер-план**; щільний keeper-файл → **RETAIN** + рядок у `KNOWLEDGE_MAP.md`.
5. Напиши **майстер-план за скелетом §5**.
6. Прибери `_harvest_inbox/` (сирі дампи не лишаються).

**Що на виході:** один документ — див. **скелет §5** (його заголовки = форма результату; можна підправити з власником ДО жнив).
...
**Рубрика (диспозиції):**
- 🟢 **HARVEST** — читати детально, витягти зерно: бачення/ідеї/відкриті питання/нові модулі/серверне, ще живе.
- 🔵 **RETAIN** — щільний keeper: **зберегти файл цілим** (не кришити на пункти), промоутнути в дім + рядок у `KNOWLEDGE_MAP.md`. Для документів, де майже все живе й цінне (маркетинг, продуктова візія, бізнес-логіка).
...
**ДВА ОКРЕМІ ТРЕКИ** (різна природа джерел):
- **Репо-доки** — технічні консультації/візії/handoff'и; триаж за списком §2.
- **Зіп-чати** (`_harvest_inbox/`, §3) — **старіший і ширший корпус**: багато **продукт/бізнес/маркетинг**, а не лише технічне. Триажити за трьома категоріями (крім технічних):
  - **Продуктові фічі** — корисні, реалізовні **незалежно від архітектури** (серверна чи ні).
  - **Бізнес-логіка** — як має працювати з точки зору **роботи адвоката**, щоб зручно.
  - **Маркетингові дослідження** — ринок, сегменти, позиціонування, контент.
  Щільні keeper'и цих категорій — **RETAIN** (напр. `context_marketing_strategy_v1` — ~95% живого зерна → зберегти цілим). Старі версії того, що вже канонічне (SaaS/DP/billing task-специфікації, старі CLAUDE.md/philosophy) — SKIP, але скімнути на **осиротілі ідеї**, що не дожили до коду.
...
- **Нові модулі ще не розпочаті:** **Canvas** (серверна дата-модель **збережена** — `docs/consultations/canvas_server_data_model.md`: таблиці + RLS, ізоляція+явний шар; візії v1.0/v1.5 теж у репо, див. `KNOWLEDGE_MAP §3`) і **Billing/CRM** (підготовка збирається; візія v1..v1_5 — у зіпах, після жнив RETAIN).

---

## 5. СКЕЛЕТ МАЙСТЕР-ПЛАНУ (форма результату — заголовки, які сесія заповнює)

> Це і є «що я матиму в результаті»: **один документ** із цими розділами. Заголовки можна підправити з власником ДО жнив. Кожен пункт — з вердиктом *зараз / на сервері / enabling / rebuild* і одним реченням чому.

```
# Майстер-план серверного переходу Legal BMS
(дата · статус · як читати)

## 1. Порядок серверної міграції (фази + блокери B1-B16)
### Фаза 0 — рішення/спайк (стек; обовʼязковий окремий compute-шар)
### Фаза 1 — Auth + проксі секретів (закриває B1/B2/B3; система стає продаваною)
### Фаза 2 — Postgres + RLS (міграція registry_data.json → таблиці)
### Фаза 3 — важкий compute на worker (B4-B11, B16)
### Фаза 4 — job-стан/resume + прибрати localStorage-tier (B12-B15)

## 2. Рефакторинг (Вісь C) — що з міграцією, що після
(App.jsx переважно розчиняється; CaseDossier/Dashboard — з міграцією; дизайн Рівень 0 — зараз)

## 3. Допилюємо vs робимо наново — ТАБЛИЦЯ
(фіча → вердикт → одне речення чому; серверна лінза «переживає/розчиняється»)

## 4. Нові модулі — коли і в якому порядку
### Canvas        (на хребет Фаз 1-2; дата-модель уже є — canvas_server_data_model.md)
### Billing / CRM (візія з зіпів + recommended_activity_event_billing_architecture)
### Кандидати з зіпів: Content Hub · Workflow engine · Telegram · Main-agent (оцінити: парк/план)

## 5. Паралельні гілки
### ЄСІТС / Court Sync (Журнал/Розбіжності/синхронізація)   ### Дизайн (3 рівні токенів)

## 6. Продукт / бізнес / маркетинг (із зіпів)
(RETAIN-keeper'и + беклог: тарифи, воронка, CAC/LTV, бренд, відеоархів-стратегія…)
```
...
**Кінець kickoff.** Сесія-жнив: почати з **§0 (завдання) → §1 (метод) → §2.1 (HARVEST) + §4 (засів)**; §2.2 — лише по названому принципу; §2.4 — не відкривати; зіпи — §3 окремим треком. Ціль — **майстер-план за скелетом §5**. Метрики/склад файлів звірено по `main` 2026-06-27.
