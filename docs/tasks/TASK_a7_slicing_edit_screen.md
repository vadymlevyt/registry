...
- **ПЕРЕГЛЯД СТОРІНОК ДЛЯ ОЦІНКИ МЕЖ (критично — без цього межі не поправити):** план показується
  не лише як дерево вузлів, а як **стрічка карток-сторінок, згрупованих по документах**.
  - Кожна сторінка = картка з **першими рядками тексту** з `page._text` (уже в session — дешево,
    миттєво, без рендеру). На межі видно сусідів послідовно: …стор.2 │**роздільник групи**│ стор.3…
    (на стор.3 читається заголовок нового документа — підтвердження межі), і так само на іншому кінці.
  - **Межа = роздільник між групами.** Правка межі = **перетягнути картку сторінки** в сусідню групу
    (зсунути межу) або **розділити** групу на сторінці — адвокат бачить, ЩО рухає. Сусіди завжди поруч.
  - **Для сканів** (кривий OCR-текст): на клік по картці — **рендер самої сторінки** (pdf.js з
    `_temp`-оригіналу), **лінькувато** (не всі сторінки одразу; на вимогу).
  - **Перевикористати патерн склейки фото** (`DpImageMergeEditor` + `services/imageDocument` grouper):
    той самий UX «картки, згруповані в документи, перетягуються між групами», лише сторінки PDF
    замість фото (Rule of Three — не вигадувати новий). Спільні елементи виносити, якщо третій споживач.
