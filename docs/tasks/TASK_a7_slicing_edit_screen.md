...
### 2.3 Виняток (ii) — дата в propose-плані
- `triagePrompt` додає в JSON-вихід `date` (`YYYY-MM-DD`|null) на документ.
- `triageStage.normalizePlan` переносить `date` у вузол плану.
- `splitDocumentsV3` (`defaultBuildMetadata`/`buildMeta`) пише `date` у `createDocument`.
- Екран показує дату на вузлі через `DatePicker`, **правиму** (AI-пропозиція = дефолт, не істина).
