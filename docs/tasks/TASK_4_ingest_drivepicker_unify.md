...
**B2 — злиття пікерів.** `DocumentProcessorV2/DrivePicker.jsx` зведено у той самий `components/DrivePicker/` (конфігурований: single/multi, modal/inline). Обидва (модалка, DP) споживають один. Старий файл видалено. 🔹

**C — DP-сценарій «просто додати».** Новий вхід у DP: додати **комбо готових файлів** (PDF/HTML/DOC/image) у будь-якій кількості, **без нарізки**. Кожен конвертується в PDF через `converterService` (переюз); зображення — через image-флоу (downscale вже є); PDF searchable — просто додається; scanned PDF — за `ocrMode`. (`skipPdfSlicing` — врахувати/не дублювати.) 🔹
