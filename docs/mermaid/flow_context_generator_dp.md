# Flow: оновлення case_context.md при додаванні документів через Document Processor

> Тип: flow-діаграма (тримати синхронно з кодом). Привід — діагностика гонки стану
> нижче (NO_FILES при увімкненому тумблері «Оновити контекст»).
...
## 🔴 НАЙСЕРЙОЗНІШИЙ НАСЛІДОК — тихе пошкодження даних на НЕпорожній справі

`NO_FILES` на порожній справі (видимий жовтий тост) — це лише **гучний** прояв
гонки. Справжня загроза — той самий race на справі, де ВЖЕ є документи:

| Сценарій | `caseData` у замиканні | Результат `generateCaseContext` | Що бачить адвокат |
|----------|------------------------|----------------------------------|-------------------|
| Порожня справа | `documents: []` | `NO_FILES` | 🟡 «Нарис не оновлено» — **видно, не шкодить** |
| Справа з документами | **старий** список (без щойно доданих) | `saved: true` по **старих** матеріалах | 🟢 «Нарис оновлено» — **виглядає як успіх** |

У другому рядку (`CaseDossier/index.jsx:771-774`) гілка `if (result?.saved)` дає
зелений тост `messages.context.updated` — система запускає OCR по застарілому
списку, зберігає `case_context.md` БЕЗ нових документів і рапортує успіх. Адвокат
не помічає: щоб виявити, треба відкрити свіжий файл і перечитати весь — чого ніхто
робити не буде. Імовірно, це **вже траплялось у попередніх тестах** (контекст
«оновлювався» зеленим тостом, але нові матеріали в нього не потрапляли).

Це переводить дефект з класу «UX-глюк на порожній справі» у клас **silent data
correctness bug** (найгірший — не сигналить). Фікс «дочекатися метаданих» не
косметичний, а **обов'язковий**: він лагодить саме невидимий випадок. Гучний
`NO_FILES` зник би як приємний побічний ефект.
