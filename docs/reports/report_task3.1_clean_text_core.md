...
**Статус:** код завершено (+ Доповнення 2: чанкування/ліміти/truncation-guard,
crash-safe долі артефактів, module у телеметрії), `npm test` зелений (1807 тестів),
...
- agentType `textCleaner` у `modelResolver.SYSTEM_DEFAULTS` → Haiku. Ескалація на Sonnet —
  через `modelPreferences` за результатами реального прогону (Брановський); Opus НЕ закладаємо.
- `logAiUsage({agentType:'text_cleaner', context:{caseId, module, operation:'clean_text',
  documentId}})` — **на КОЖНУ пачку** (точна вартість; пачкування → кілька викликів/документ).
- `activityTracker.report('agent_call', ...)` — оплачувана ДІЯ адвоката **РАЗ на документ**
  (не на пачку), лише при `billAsUserAction:true` (кнопки/ACTION у 3.2). DP передає `false`.
  Розділення: токени per-call (logAiUsage), дія per-document (activityTracker) — інакше
  пачкування над-рахувало б одну дію як N.

---

## Доповнення 2 — чанкування, ліміти, truncation-guard, crash-safe (2026-06-01)

**A. Пачкування + ліміти (`flow_clean_text_chunking.md`):** прибрано спадковий фіксований
`max_tokens 8000`. Два РІЗНІ бюджети: ВХІД (вікно Haiku 200K) і ВИХІД (стеля Haiku 4.5 =
**64K**, API відхилить >64000). Документ чиститься **ПАЧКАМИ сторінок**: `packPagesIntoBatches`
пакує сторінки за вихідним бюджетом (`MAX_BATCH_OUTPUT_TOKENS=24000`; оцінка токенів сторінки
= `_text.length / 2` для кирилиці; лік сторінок у пачці плаває за щільністю). На кожну пачку —
КРОК1 конденсатор + КРОК2 поліш; склейка через роздільник сторінок `---` (межі збережено).
`max_tokens` рахується від обсягу пачки (`maxTokensForEstimate`, стеля 64000). Вхідний
`slice(120000)` піднято до `INPUT_CHAR_CAP=200000`.

**Truncation-guard:** `polishToMarkdown` детектує `stop_reason==='max_tokens'` → `truncated`.
`cleanDocument` при обрізанні пачки з >1 сторінки **ділить пачку навпіл і повторює**
(halving-retry, до 1 сторінки). Якщо й одна сторінка обрізалась — **деградація**.

**B. Долі артефактів — умова + порядок:** руйнівні кроки ТІЛЬКИ при **повному успіху**
(AI відпрацював, не обрізало, не fallback на чернетку, є непорожній MD). При будь-якій
проблемі (`degraded`) — `.txt` І `.layout` **НЕДОТОРКАНІ**, `.md` НЕ фіналізується,
повертається `{ok:false, degraded:true, needsRecleaning:true, warning}`. Crash-safe порядок
на успіху: **1)** `saveMarkdown` → **2)** `updateDocumentMeta` (факт успіху ДО руйнування) →
**3)** `moveRawTxtToArchive` → **4)** `deleteLayout` (надмножина `.txt`, паливо повторної
очистки — видаляємо ОСТАННІМ). У DP пост-кроці деградація → `decision text_clean_failed`.

**C. `module` у телеметрії:** `cleanDocument`/`polishToMarkdown` приймають `module` і прокидають
у `logAiUsage` context + `activityTracker`. DP пост-крок → дефолт `MODULES.DOCUMENT_PROCESSOR`;
кнопки 3.2 / реєстр 3.3 передаватимуть `MODULES.CASE_DOSSIER`.

**D. Модель:** дефолт Haiku через `resolveModel('textCleaner')` — лишено. Ескалація на Sonnet —
нотатка (не хардкод), за реальним прогоном.

---

## Тести (до/після)

База до TASK: **1753**. Після: **1807** (+54).

- **Unit `tests/unit/cleanTextService.test.js`** (новий, 33 кейси): конденсатор на
  ...
  polishToMarkdown (нема ключа, JSON-парс, C7 agentType/operation/module, `max_tokens`
  від обсягу + стеля 64000, переданий maxTokens, `stop_reason=max_tokens`→truncated,
  AI кинув→draft, не-JSON→plain, НЕ викликає activityTracker); cleanDocument (скоуп-гард,
  crash-safe порядок md→meta→txt→layout через invocationCallOrder, **пачкування** →
  кілька AI-викликів + склейка, **truncation-guard** одна сторінка обрізана→джерела
  недоторкані ok:false, AI-помилка→degraded, нема ключа→degraded, txt-fallback без
  deleteLayout, NO_SOURCE, activityTracker РАЗ на документ при true / не при false,
  module прокинуто, NO_DOCUMENT).
...
- **Новий борг (винос геометрії layout у спільний `layoutGeometry`):** Rule of Three —
  третій споживач. На гілці записаний як `#43`, АЛЕ на main `#43`=layout-as-source,
  `#44`=MODEL_PRICING, `#45`=Batches (НЕ цей TASK). **ПРИ ФОЛДІ перенумерувати запис
  гілки у наступний вільний (#46), main-ові #42-#45 НЕ затирати.**
