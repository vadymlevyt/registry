...
**Корекція плану фіксу:** головний крок — НЕ «floor у memoryMonitor», а **перевірка
реальної ваги шматка після `splitPdf`**: якщо нарізка не зменшила (шматок ≥ ліміту) —
...
вторинні щодо цього інциденту. Діаграма для адвоката: `docs/mermaid/flow_dp_regression_40mb_diagnosis.md`.
