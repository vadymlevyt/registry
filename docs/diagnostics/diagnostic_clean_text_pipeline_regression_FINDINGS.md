...
## 0-bis. ПОПРАВКА ПІСЛЯ УТОЧНЕННЯ АДВОКАТА (2026-06-02, головне)

Адвокат повідомив два факти, що **уточнюють корінь** (але НЕ міняють головного
висновку — clean-text і код невинні):

1. **Файл, що падає: 79 МБ / 335 сторінок = 0,24 МБ/стор.** Версія §5.2 цього звіту
   («floor → 5 важких сторінок по >8 МБ = >40 МБ») для ЦЬОГО файлу **НЕВІРНА** —
   арифметика не сходиться. Floor-дефект реальний, але це **не** причина цього падіння.
2. **Той самий том раніше важив 200 МБ і нарізався (335 стор.). Адвокат стиснув його
   до 79 МБ — і він зламався.** Стиснення — вирішальна нова змінна.

**Уточнений корінь:** «більше 40 МБ» на файлі 0,24 МБ/стор. можливе ЛИШЕ якщо нарізка
на сторінки **перестала зменшувати розмір шматка**. Нарізка (`documentBoundary/splitPdf.js:42`)
вирізає сторінки через pdf-lib `copyPages`, яка **копіює сторінки РАЗОМ з усіма
ресурсами, на які вони посилаються**. Зовнішнє стиснення (200→79 МБ, image-downsample)
переструктурувало PDF так, що сторінки **ділять спільні ресурси** → `copyPages` навіть
5–25 сторінок **тягне майже весь файл** → шматок ~40–79 МБ → `documentAi:298` throw.
До стиснення сторінки були незалежні → шматки справді маленькі → працювало.

- **Прояв А (файл 79 МБ):** шматок ≥40 МБ → «більше 40 МБ» → падіння job.
- **Прояв Б (файли 18–42 МБ):** шматок не зменшився, але <40 МБ → OCR одним суцільним
  блоком → 1 документ, ОБРОБЛЕНІ порожні. (Цей бік менш певний — потребує підтвердження
  прогоном; див. інструментування.)

**Узгодженість із рештою звіту:** код нарізки (`splitPdf`/`chunkManager`/`copyPages`)
**не змінювався** з робочого стану (§4) — зламав не код, а **новий вхід** (стиснений PDF),
що зустрівся з пре-існуючою межею copyPages-нарізки. Тому **clean-text і код-регресія так
само спростовані** (§2, §7), лише проксимальний механізм — стиснення, а не floor §5.2.

**Підтвердження за 1 прогін:** дати ОРИГІНАЛ 200 МБ (нестиснений) — має нарізатись як
раніше. Якщо так — діагноз доведено. Додатково інструментувати: лог `sizeMB` кожного
шматка після `splitPdf` і `pageCount` від `pdfInfo`.

**Корекція плану фіксу (Проблема A):** головний крок — НЕ «floor у memoryMonitor», а
**перевірка реальної ваги шматка після `splitPdf`**: якщо нарізка не зменшила (шматок ≥ ліміту) —
ділити далі / рендерити сторінки в зображення (обхід спільних ресурсів через pdfjsLocal),
або чесний toast «файл стиснутий так, що не ріжеться — дайте оригінал», а не `EXECUTOR_THREW`.
Floor-дефект (§5.2/§9) і `catch{}` (§9) лишаються валідними дотичними фіксами, але вони
вторинні щодо цього інциденту.

---

### ДРУГЕ УТОЧНЕННЯ АДВОКАТА — Проблема B (нестиснені файли → 1 документ)

Адвокат уточнив далі: **нестиснені файли, точно ті самі, що раніше різались, тепер дають
1 документ** (вхід=вихід, ОБРОБЛЕНІ порожні). Стиснення (Проблема A) це НЕ пояснює — це
**окрема, справжня регресія**.

**Що встановлено по коду (static):**
- Межі документів визначає **AI-виклик Haiku** (`aiTriage` → `analyzeTriageViaToolUse`) на
  основі «паспорта» (`passportOf` → `resolveBoundaryText` → `buildCompactTriagePassport` /
  `buildPagedText` / flat plainText).
- **Весь цей шлях НЕ змінювався** з робочого стану: `pageMarkers.js`, `triagePrompt.js` —
  ідентичні (`git diff ac6a135 ab6086e` порожній); `analyzeTriageViaToolUse.js` — лише
  `max_tokens 4000→16000` + лог (це ПОКРАЩЕННЯ, документований fix проти «здавальницького
  плану», не регресія). OCR/чанк-ядро — байт-ідентичне (§4).
- **Задокументована вразливість** (`pageMarkers.js:193, 336-337`): повний посторінковий
  паспорт **переповнює вікно Haiku на 200-250+ стор.** → AI повертає вироджений план. Для
  цього й створено `buildCompactTriagePassport`. Файли адвоката — томи 200-335 стор.
- **`isPagedLayout` (pageMarkers.js:28)** вимагає `pages.length === expectedPageCount` —
  **строга рівність**: будь-яке розходження кількості сторінок layout vs `pdfInfo.pageCount`
  (одна порожня/пропущена сторінка від OCR) викидає ВСІ посторінкові мітки → fallback на
  flat-текст без меж → AI повертає 1 документ.
- **Halt-шлях НЕ дає 1 документ:** на `triage_whole_volume` (halt) диригент персистить
  **0** документів (`documentPipeline.js:505-511`, `ok: documents.length>0 && !stoppedAt`).
  Тож «1 документ у 01» адвоката — це або (а) AI повернув **валідний 1-doc план** (том <70
  стор. або не покриває критерій `isDegeneratePlan`), або (б) тумблер **`skipPdfSlicing`**
  («Просто додати файли») був ON (1 add_as_is/файл). `skipPdfSlicing` default OFF і **НЕ
  персиститься** (`useState(DEFAULT_SETTINGS)`, не localStorage) — тож ON лише якщо адвокат
  увімкнув у поточній сесії.

**Чесна межа:** увесь релевантний код незмінний, а різник — **недетермінований AI**. Тому
різниця «раніше різало / тепер ні» лежить у **рантайм-умовах**, які НЕ відтворюються в
пісочниці (нема файлів/Drive/API): (1) паспорт завеликий для цих томів → Haiku здається;
АБО (2) посторінкова карта (layout) приходить неповною → flat-fallback → 1 документ; АБО
(3) тумблер skipPdfSlicing. **Точний тригер вимагає ОДНОГО інструментованого прогону** —
рівно те, що ця діагностична сесія мала б зробити, але не може без живого середовища.

**План фіксу (Проблема B) для наступної сесії:**
1. **Інструментувати + 1 прогін** на реальних файлах: лог уже є (`[Triage] artifacts=N
   pages=P input=… output=…`). Додати лог `isPagedLayout`, `layout.pages.length` vs
   `pageCount`, довжину паспорта, стан `skipPdfSlicing`. Це за один прогін розрізнить 3 гілки.
2. Якщо паспорт-overflow → підсилити `buildCompactTriagePassport` / `passportOptsForBudget`
   для 200+ стор. (агресивніша компактизація або посегментний triage).
3. Якщо `isPagedLayout` мисфайр через ±1 сторінку → пом'якшити строгу рівність
   (`pages.length === pageCount` → толерантність / використати наявні мітки частково).
4. Обхід для адвоката зараз: різати том на частини по ~70-100 стор. і обробляти окремо.

Діаграма для адвоката: `docs/mermaid/flow_dp_regression_40mb_diagnosis.md`.

---

### ТРЕТЄ УТОЧНЕННЯ АДВОКАТА — РЕТРАКЦІЯ механізму Проблеми A

Адвокат уточнив: файл 79 МБ = той самий, що 200 МБ, лише **стиснений простим
unpack → downscale зображень → repack**; УСІ файли системи — **скани (зображення) у PDF**,
по одному зображенню на сторінку.

**Ретракція:** гіпотеза §0-bis «стиснення створило СПІЛЬНІ ресурси, тож `copyPages` тягне
весь файл» **НЕ тримається для простих сканів** — downscale незалежних посторінкових
зображень НЕ створює спільних ресурсів; вирізані 25 сторінок мали б давати малий шматок.
Адвокат справедливо це відкинув. **Це вже ТРЕТЯ моя конкретна гіпотеза механізму (після
floor §5.2 і shared-resources §0-bis), і я знімаю її так само.**

**Чесна підсумкова позиція (static-межа досягнута):** увесь релевантний код
(нарізка `splitPdf`/`chunkManager`, розпізнавання `documentAi`, AI-різник `triageStage`/
`analyzeTriageViaToolUse`, паспорт `pageMarkers`) **не змінювався** з робочого стану
(доведено diff-ами). Отже регресія — **рантаймова**, і назвати точний механізм **зі
статичного коду неможливо**. Three конкуруючі кандидати, які РОЗРІЗНИТЬ один інструментований
прогін:
1. **pdfInfo (pdf-lib `getPageCount`) повертає невірну к-сть сторінок** цього PDF →
   `chunkManager.planChunks` будує чанк на (майже) весь файл → >40 МБ throw / 1 «сторінка».
2. **`splitPdf` (pdf-lib `copyPages`+`save`) не зменшує шматок** для PDF цього репакера →
   блок завеликий. (Перевіряється логом `sizeMB` кожного part.)
3. **OCR дає порожній/неповний посторінковий `_text`** → паспорт flat → AI-різник → 1 документ.

**Обовʼязковий КРОК 1 наступної сесії (у реальному середовищі, НЕ пісочниця):** додати
тимчасові логи у `streamingExecutor.streamFile` (перед `processChunk`: `chunk.startPage-endPage`,
`chunkBytes.byteLength`, `pageCount` від planChunks) і в `chunkManager.planChunks`
(`pdfInfo.pageCount`, `per`) і прогнати ОДИН файл адвоката. Один прогін відповість, який із
3 кандидатів — справжній. Без цього будь-яке «це через X» — здогад (підтверджено тричі).

**Чого пісочниця НЕ може:** немає файлів адвоката, Drive OAuth, Anthropic API ключа і
планшетного середовища — рантайм OCR/нарізки не відтворюється. Тому ця сесія чесно
зупиняється на: (екзонерація clean-text + код незмінний + 3 кандидати + готова інструментація),
а не на хибно-впевненому «причина — X».
