...
## 3. Canvas (модуль — НЕ будувався) 🟢
| Файл | Що там |
|------|--------|
| `docs/consultations/context_canvas_constructor_v1.md` | Базова візія v1.0: філософія «помічник пише — ти надиктовуєш», канва+агент, три стартові точки, шаблони, фрагменти, тули, версіонування, експорт .docx, phases, білінг, позиціонування. |
| `docs/consultations/canvas_context_model_v1_5.md` | v1.5 (Розділ 21): мульти-таб, три шари контексту, прив'язані в'юер-документи, два режими в'юера, персистентність, файлова система адвоката, форматування/експорт (PDF≈один-в-один / Word=структура), провайдерний редактор, шаблони-прапорець, «усе є тул», Канва→ЄСІТС. **NB: розділи зберігання 21.13-21.16 писались під клієнт+Drive — переформульовані під сервер ↓.** |
| `docs/consultations/canvas_server_data_model.md` | **Серверна дата-модель** (Postgres): `canvas_documents/_versions/_shares/_attached/_sessions` + RLS; ізоляція за замовчуванням + явний шар; звʼязок із `team[].permissions`; що переживає/переписується. |
