...
### [2026-05-26] API-параметри прибирання даних впливають на downstream pipeline

**Контекст:** TASK 26.05 (`TASK_documentai_limit_40mb_imageless.md`) додав
`imagelessMode: true` у Document AI request з метою зменшити response size
(превентивний фікс пам'яті для iPad heap). Параметр документований в офіційному
Google Discovery API (`GoogleCloudDocumentaiV1ProcessRequest.imagelessMode`),
опис прозорий: «Option to remove images from the document».

**Що пішло не так:** `imagelessMode: true` прибирає з response не тільки
сам `image` base64, а й залежні від нього поля
(`imageQualityScores.qualityScore`, `visualElements` — печатки/підписи).
У нашому compact passport (`pageMarkers.js:compactDigest`) ці поля — частина
сигнальної сітки меж документів, яку Triage AI Haiku зважує разом. Втрата
2-3 сигналів на томах 285+ стор. (де паспорт і так розріджений через
rich-cap 70 стор.) → Triage здається → видає мінімальний план «1 doc /
pageCount=1 / тип null». Том 1 (335 стор / 207 МБ), який стабільно
нарізався на 18-21 doc до полудня 26.05 (двічі поспіль на планшеті
адвоката), перестав нарізатися.

**Корінь рішення:** ввели API-перемикач прибирання даних, не перевіривши
які поля downstream pipeline СПОЖИВАЄ і які з них залежать від цього
перемикача. «Зрозуміло з опису» ≠ «безпечно для нашого pipeline».

**Правило на майбутнє:**
> Перед увімкненням будь-якого API-параметра типу `*Mode`, `disable*`,
> `skip*`, `exclude*`, `*Less` — провести grep по downstream коду:
> «які саме поля response ми споживаємо?». Якщо хоч одне з них
> **потенційно** залежить від прибраних даних — НЕ вмикати без реального
> тестування на представницькому наборі (через тестовий API виклик з і
> без параметра, порівняти response shape).

**Пов'язано з правилом #11** (однозначність): `imagelessMode` несе насправді
два сенси — «прибрати image base64» (очікуване, з опису) і «прибрати залежні
поля» (приховане, з downstream поведінки). API-документація не розкриває
другий — лише downstream поведінка показує.

**Не вмикати знов** без явного TASK з валідаційними даними протилежного
знаку і явним списком які поля треба зберегти. Відкат — `TASK_revert_imagelessmode.md`
(26.05.2026); 40 МБ guard з того ж дня залишається працювати окремо.
...
### [2026-04-26] HTML з ЄСІТС — windows-1251 без `<meta charset>`
ЄСІТС видає HTML у кодуванні windows-1251 без явного `<meta charset>`.
TextDecoder('utf-8') на таких файлах дає абракадабру.

Автодетекція в `src/services/ocr/pdfjsLocal.js`:
1. Спершу читаємо перші 4 КБ як latin1 і шукаємо `charset=...` в `<meta>`.
2. Якщо є — використовуємо вказане кодування (`cp1251`/`win-1251` мапимо на `windows-1251`).
3. Якщо немає — пробуємо UTF-8 і рахуємо replacement chars (U+FFFD).
   Понад 1% replacements = не UTF-8 → fallback на windows-1251.
4. Парсинг через нативний DOMParser, видалення `<script>`/`<style>`,
   `body.innerText` як результат.

Правило: для HTML з державних систем України — завжди автодетекція кодування.

---

### [2026-05-02] batch_update — пакетне виконання дій агентом
- Реалізовано як композитна дія в ACTIONS (src/App.jsx)
...
- В sendChat: окремий if-блок ПЕРЕД каскадом одиничних дій
- Резолвер: case_name → caseId, deadline_date/hearing_date → id сутності
- Тестова матриця: 4 сценарії в TASK.md від 02.05.2026
