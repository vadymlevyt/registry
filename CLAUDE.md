...
**Сценарій:** адвокат → «Електронний суд» → «Імпорт» → копіює промпт → вставляє у Claude for Chrome (sidebar браузера) → агент обходить кабінет ЄСІТС, фільтрує справи за роком (25/26), повертає JSON envelope → адвокат вставляє у textarea «Обробити». Legal BMS створює нові справи (`origin='ecits_import'`, name префікс `[ЄСІТС]`), оновлює існуючі (за нормалізованим `case_no` через `src/services/ecits/caseNoKey.js` — TASK ecits_identity_by_caseno), додає засідання 2026 (`hearing.source='court_sync'`). Підсумок у ResultCard.
...
**Розширений `create_case`** (`actionsRegistry.js`): підтримує два формати — legacy `({ fields })` і плоский `({ name, case_no, ..., origin, ecitsState, parties, processParticipants })`. Плоскі ключі мають пріоритет над `fields`. Дедуплікація (TASK ecits_identity_by_caseno): якщо нормалізований `params.case_no` (через `caseNoKey.normalizeCaseNoKey` — trim/lower/без пробілів/без кінцевого суфікса-літери) збігається з уже існуючою справою — повертає `{ success: false, error: 'duplicate_case_no', existingCaseId }`. Використовує `ensureCaseSaasAndEcitsFields` (v7+v9 канонічний дефолт) замість лише `ensureCaseSaasFields`.
...
4. **`scenarioProcessor.js`** (`src/services/ecits/scenarioProcessor.js`): спільна функція для UI («Обробити» в ImportTab) і майбутнього extension. DI: приймає `{ executeAction, agentId, transport, getCases, getTenant, appendScenarioHistoryEntry, onProgress }`. Кроки: `validateEnvelope` → для кожної ecitsCase: дедуплікація через нормалізований `case_no` (`caseNoKey.normalizeCaseNoKey`) → `create_case`/`update_case_ecits_state` → `add_hearing` для нових засідань → `appendScenarioHistoryEntry`. `ecitsCaseId` з envelope приймається-але-ігнорується. Вхідний guard: skip лише при відсутності `case_no` (раніше — за відсутності `ecitsCaseId`; TASK ecits_identity_by_caseno). Повертає `{ casesCreated, casesUpdated, hearingsAdded, skipped, errors, warnings, scenarioRunId }`. Read-канал — живий: App.jsx подає `getCases:()=>casesRef.current` (ref оновлюється і в `setCasesWithRef`-апдейтері, і в пост-render useEffect), щоб within-run read-after-write працював і дедуп не дублював у межах одного прогону.
