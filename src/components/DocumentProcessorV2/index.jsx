...
export default function DocumentProcessorV2({ caseData, onExecuteAction, driveConnected, aiUsageSink }) {
  ...
  const startProcessing = async () => {
    ...
    // Маршрутизація сценаріїв (детермінований вибір НА ВХОДІ). Тумблер
    // «Нарізати / склеїти» (sliceVolume) ON → система САМА обирає за ТИПОМ входу:
    //   • all-image + ON → DP image-merge editor (склеїти фото в документ).
    //   • all-PDF / mix без фото + ON → стрім-шлях з AI Triage (нарізати PDF-том
    //     на окремі документи).
    //   • мікс photo+PDF + ON → toast (нарізка/склейка-мікс поза scope; для
    //     комбо адвокат вимикає тумблер).
    // OFF (дефолт) → ЗАВЖДИ addFiles (просто додати файли як є, ВКЛ. all-image):
    //   per-file, НІКОЛИ нарізка/стрім-блоки/склейка — незалежно від типу і
    //   OCR-режиму. Повний OCR розпізнає кожен файл ЦІЛИМ документом (layout у
    //   02), але НЕ ріже на шматки. «Без OCR» — лише 01.
    // Склейка фото живе ЛИШЕ в режимі sliceVolume; у просто-додати (дефолт)
    // all-image йде в addFiles, а не в склейку.
    ...
    // Дефолт. Нарізка/склейка вмикається свідомо тумблером «Нарізати / склеїти»:
    ...
    if (!useAddAsIs && hasAnyImage() && hasAnyNonImage()) {
      // Мікс фото+PDF у режимі НАРІЗКИ (тумблер «Нарізати / склеїти» ON) — поза scope.
      // Підказуємо вимкнути тумблер для комбо без нарізки/склейки (просто-додати).
      toast.warning('Мікс фото + PDF: вимкніть «Нарізати / склеїти» для комбо', {
        description: 'Нарізка/склейка працює лише для чистих наборів. Для змішаного набору — вимкніть тумблер «Нарізати / склеїти».',
      });
      return;
    }
      ...
      <div className="dpv2-body">
        ...
        <section className="dpv2-zone" aria-label="Налаштування">
          ...
          <div className="dpv2-settings-group">
            ...
            <Toggle
              label="Нарізати / склеїти"
              description="Сканований PDF-том → окремі документи; фото → склеїти в документ. Вимкнено — просто додати файли як є."
