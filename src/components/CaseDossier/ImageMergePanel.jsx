...
export const ImageMergePanel = forwardRef(function ImageMergePanel(
  ...
  const handleStartProcessing = async () => {
    ...
    if (files.length > MAX_IMAGES_WARN) {
      // Б6: confirm для 50+ зображень — у наступному комміті B.6 робимо
      // повноцінний confirm dialog. Зараз — toast warning.
      toast.show(`Великий обсяг (${files.length} зображень) — обробка може зайняти 1-2 хвилини`);
    }
...
function ProcessingView({ progress }) {
  const PHASE_LABELS = {
    ...
    heic: 'Конвертація HEIC',
    ocr: 'Розпізнавання тексту',
    sort: 'Сортування агентом',
    rotate: 'Корекція орієнтації',
    pdf: 'Створення PDF',
  };
  const label = PHASE_LABELS[progress.phase] || 'Обробка...';
  ...
  return (
    <div className="image-merge-panel__processing">
      ...
      <div className="image-merge-panel__processing-label">{label}</div>
      <div className="image-merge-panel__processing-progress">
        {progress.total > 0
          ? `${progress.done} / ${progress.total} (${pct}%)`
          : '...'}
      </div>
      <div className="image-merge-panel__processing-bar">
        <div
