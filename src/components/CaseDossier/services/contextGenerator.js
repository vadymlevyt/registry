...
export async function generateCaseContext(params) {
  ...
  console.log(`[contextGenerator] OCR джерело: ${allFiles.length} файлів`, allFiles.map(f => `${f.name} (${f.sourceLabel})`));
  onProgress(`Знайдено ${allFiles.length} файлів. Запускаю обробку...`);
  ...
  return {
    ...
    stats: { count: textDocs.length, fromCache: cacheHits, failed: failed.length },
  };
}
