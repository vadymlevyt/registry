...
//       warnings: [                         // підозрілі сторінки (інший документ)
//         { index: 4, reason: "Сторінка з іншого документа: інша тематика" }
//       ],
//       duplicates: [                       // групи дублікатів (TASK B fix 1)
...
//     }
//
//   duplicates — групи зображень що відображають ОДНУ І ТУ САМУ сторінку
//   (різні фото, повторні скани). group — масив original indices з ≥2 елементами.
//   recommended — один з group, який агент рекомендує залишити (кращий OCR,
//   менше спотворень). У preview всі group виділяються жовтою рамкою, а
//   recommended додатково отримує зелений border + ярлик «рекомендую залишити».
//
...
2. Виявити ДУБЛІКАТИ — два чи більше зображень які показують ОДНУ І ТУ САМУ сторінку (повторне фото з іншого ракурсу, повторний скан). Сигнал — практично ідентичний OCR-текст або текст з тими самими ключовими реквізитами і фрагментами.
3. Виявити підозрілі сторінки які НЕ належать до цього документа (інша тематика, інші реквізити, інший суд, інша справа).
4. Запропонувати коротку функціональну назву документа.
5. Зазначити пропущені сторінки якщо помітно (наприклад нумерація йде 1-2-4 — пропущено 3).
...
СИГНАЛИ ДЛЯ ДУБЛІКАТІВ:
- Практично однакові перші 200-300 символів тексту (з поправкою на OCR-шум)
- Один і той самий заголовок, дата, номер справи, ПІБ
- Однакові завершальні рядки
- Однакові резолюції/висновки
- Різна тільки якість OCR (одне фото чіткіше, інше зі спотвореннями)
- Та сама довжина тексту ± 10-15%

ЯК ВИБРАТИ recommended З ГРУПИ ДУБЛІКАТІВ:
- Довший OCR-текст (більше розпізнаних символів = чіткіше фото)
- Менше очевидних артефактів розпізнавання (символи-замінники, обірвані слова)
- Якщо однаково — менший index (раніше зробленне фото зазвичай якісніше)
...
JSON SCHEMA:
{
  ...
  "duplicates": [
    {"group": [<original_index>, <original_index>, ...], "recommended": <original_index>, "reason": "коротке пояснення чому recommended кращий"}
  ],
  "warnings": [
...
- Дублікати ТЕЖ лишаються в order (вирішить адвокат через кнопку «Залишити рекомендований»)

ПРАВИЛА для duplicates:
- ТІЛЬКИ коли впевнений що це справді одна і та сама сторінка (>80% збіг тексту)
- group має мати ≥ 2 елементів
- recommended ОБОВʼЯЗКОВО присутній у group
- Різні сторінки одного документа з однаковими колонтитулами — НЕ дублікати
- Якщо дублікатів немає — порожній масив []
...
- НЕ попереджай про дублікати (для них окреме поле duplicates)
...
export async function sortImages(items, options = {}) {
  ...
  if (Array.isArray(parsed.duplicates)) {
    for (const d of parsed.duplicates) {
      ...
      const cleanGroup = [];
      for (const v of d.group) {
        if (!Number.isInteger(v)) continue;
        if (!allowedIndices.has(v)) continue;
        if (seenInDuplicate.has(v)) continue;
        if (cleanGroup.includes(v)) continue;
        cleanGroup.push(v);
      }
      ...
      const reason = typeof d.reason === 'string' && d.reason.trim()
        ? d.reason.trim()
        : 'Дублікат тієї самої сторінки';
      for (const v of cleanGroup) seenInDuplicate.add(v);
