...
import { callAPIWithRetry as defaultCallAPIWithRetry, callAPIStreaming as defaultCallAPIStreaming } from './toolUseRunner.js';
...
// ── Формат виходу AI-поліша (V2-B2 «Спосіб C») ──────────────────────────────
// Замість JSON-обгортки {markdown, attentionNotes} модель віддає СПЕРШУ голий
// Markdown (з інлайн-мітками ==…== як у V2-C), потім окремий рядок-роздільник
// ATTENTION_SEPARATOR, потім JSON-масив поміток. Це робить стрім тривіальним:
// показуємо все ДО роздільника як готовий Markdown (без JSON-обгортки на екрані).
// Залізні правила вірності (Чистий rules 1–9) НЕ змінені — лише обгортка виходу.
// Graceful degrade: роздільника нема → весь вихід = markdown, нотатки = [].
// Один сенс (#11): єдиний маркер «тут закінчився документ і почались помітки».
const ATTENTION_SEPARATOR = '---ПОМІТКИ---';

// idle-timeout стріму clean_text: макс. пауза БЕЗ токенів (не загальний час).
// Поки токени течуть — не абортимо (V2-B2 §2). 60с тиші = обрив → ретрай пачки.
...
// Витягти JSON-масив [...] depth-counter (поважає рядки) — для хвоста поміток
// після роздільника. null якщо валідного масиву нема.
...
// Розбір виходу поліша «Спосіб C»: markdown [+ ATTENTION_SEPARATOR + JSON-масив
// поміток]. Роздільника нема → весь текст = markdown, нотатки = [] (graceful
// degrade — модель пропустила роздільник; вірність тексту збережена, лише без
// чипів уваги). Форма нотатки — { note } (page прибрано у V2-C — прив'язка за
// порядком мітки). Толерантно приймає й хвіст-обгортку {attentionNotes:[...]}.
function parsePolishOutput(rawText) {
  const raw = String(rawText || '');
  const idx = raw.indexOf(ATTENTION_SEPARATOR);
  if (idx < 0) {
    return { markdown: raw.trim(), notes: [] };
  }
  const markdown = raw.slice(0, idx).trim();
  const tail = raw.slice(idx + ATTENTION_SEPARATOR.length);
  let arr = extractJsonArray(tail);
  if (!Array.isArray(arr)) {
    const obj = extractJsonObject(tail);
    arr = obj && Array.isArray(obj.attentionNotes) ? obj.attentionNotes : null;
  }
...
// Markdown «поки що» для стріму: показуємо все ДО роздільника. Гасимо ХВОСТОВИЙ
// частковий роздільник (напр. «---ПОМІ») щоб JSON-хвіст не мигав на екрані.
// Поріг 4 символів (>= «---П») — щоб плоский markdown-роздільник «---» (межа
// сторінок / горизонтальна лінія) НЕ обрізався помилково під час потоку.
export function streamingMarkdownView(accumulated) {
  const raw = String(accumulated || '');
  const idx = raw.indexOf(ATTENTION_SEPARATOR);
  if (idx >= 0) return raw.slice(0, idx).trimEnd();
  for (let n = ATTENTION_SEPARATOR.length - 1; n >= 4; n--) {
    if (raw.endsWith(ATTENTION_SEPARATOR.slice(0, n))) {
      return raw.slice(0, raw.length - n).trimEnd();
    }
  }
  return raw;
}
...
Поверни ВИКЛЮЧНО сам очищений документ у Markdown — без JSON, без обгорток, без роздільників, без списку поміток. Лише текст документа.
...
9. ПОЗНАЧКИ УВАГИ. Коли ти НЕ певен у фрагменті АБО посунув висмикнутий OCR-фрагмент у інше місце — обгорни ту саму фразу В ТЕКСТІ маркером ==фраза== (рівно дві = до і після, дослівно, БЕЗ зміни слів усередині) І додай той самий короткий фрагмент + причину у СПИСОК ПОМІТОК (нижче). Маркер — це лише позначка для адвоката (звірити зі сканом), він НЕ міняє слова. ПОРЯДОК ==міток== у тексті МУСИТЬ збігатися з ПОРЯДКОМ записів у списку поміток (перша мітка ↔ перший запис). Не зловживай — постав мітку лише там, де є реальний сумнів чи переміщення. Якщо все певне — жодних == і список поміток порожній.
...
1) СПОЧАТКУ — сам документ у Markdown (з ==мітками== де треба). БЕЗ JSON, без обгорток.
2) ПОТІМ, якщо є помітки — окремий рядок рівно: ${ATTENTION_SEPARATOR}
3) ПІСЛЯ роздільника — JSON-масив поміток у тому ж ПОРЯДКУ, що ==мітки==:
[ { "note": "<фрагмент + що привернуло увагу>" } ]
Якщо поміток немає — НЕ став роздільник і нічого після документа не пиши. НЕ став роздільник ${ATTENTION_SEPARATOR} всередині документа — лише один раз, після всього тексту, перед списком поміток.
...
export async function polishToMarkdown({
  ...
  maxTokens = null,
  aiUsageSink = null,
  // V2-B2 — стрім (opt-in): коли onStreamDelta — функція, поліш іде стрімовим
  // шляхом (callAIStream) і емітить markdown що НАРОСТАЄ (вже без JSON-хвоста).
  // Без неї — нестрімовий callAI (як раніше; інші споживачі не зачеплені).
  onStreamDelta = null,
  // DI-шви (дефолти — реальні; тести стабують):
  callAI = defaultCallAPIWithRetry,
  callAIStream = defaultCallAPIStreaming,
  ...
  const messages = [{ role: 'user', content: buildPromptForMode(draftStr, fileName, mode) }];
  const useStreaming = typeof onStreamDelta === 'function';
  let res;
  try {
    if (useStreaming) {
      // Стрім: idle-timeout (скид на дельті) замість total; емітимо markdown ДО
      // роздільника (streamingMarkdownView ховає JSON-хвіст поміток).
      res = await callAIStream({ model, max_tokens, messages }, {
        apiKey,
        idleTimeoutMs: STREAM_IDLE_TIMEOUT_MS,
        onDelta: (_chunk, accumulated) => onStreamDelta(streamingMarkdownView(accumulated)),
      });
    } else {
      res = await callAI({ model, max_tokens, messages }, { apiKey, requestTimeoutMs });
    }
  } catch (err) {
    return {
      markdown: draftStr,
  ...
  // V2-B2 «Спосіб C»: вихід = markdown [+ роздільник + JSON-масив поміток].
  // Роздільника нема → весь текст = markdown (graceful degrade). Форма нотатки —
  // { note } (V2-C: page прибрано — прив'язка за ПОРЯДКОМ ==мітки==).
  const { markdown: parsedMd, notes } = parsePolishOutput(rawText);
  if (parsedMd && parsedMd.trim()) {
    return {
      markdown: parsedMd,
  ...
  // Порожній вивід — fallback на чернетку (краще ніж нічого).
...
export async function cleanDocument({
  ...
  mode = 'digest',
  onProgress = noop,
  // V2-B2 — стрім (opt-in): емітить markdown ЦІЛОГО документа що НАРОСТАЄ
  // (готові пачки + жива пачка). Без неї — нестрімовий шлях (інші споживачі/агент).
  onStreamDelta = null,
  aiUsageSink = null,
  ...
  // V2-B2 — стрім (opt-in). emit(batchMd) прокидає markdown ЦІЛОГО документа
  // що наростає: ГОТОВІ пачки (completedPrefix) + жива пачка. completedPrefix
  // оновлюється у циклі ПІСЛЯ кожної завершеної пачки.
  const streaming = typeof onStreamDelta === 'function';
  let completedPrefix = '';
  const emitFor = streaming
    ? (batchMd) => {
        try { onStreamDelta(completedPrefix ? completedPrefix + PAGE_SEP + batchMd : batchMd); }
        catch { /* консумер не валить очистку */ }
      }
    : null;

  // Спільний поліш-виклик для пачки/тексту (КРОК 2), з оцінкою max_tokens.
  // emit (опц.) — стрімовий callback однієї пачки (markdownView без JSON-хвоста).
  const polishOne = (draftStr, estTok, emit) => polishToMarkdown({
    draft: draftStr,
    fileName,
    apiKey,
    caseId,
  ...
  async function cleanBatch(batchPages, emit) {
    ...
    const r = await polishOne(draftStr, estTok, emit);
    if (r.truncated && batchPages.length > 1) {
      const mid = Math.ceil(batchPages.length / 2);
      const a = await cleanBatch(batchPages.slice(0, mid), emit);
      const b = await cleanBatch(batchPages.slice(mid), emit);
      return {
  ...
  if (usedLayout) {
    ...
    for (const batch of batches) {
      ...
      // completedPrefix = вже готові пачки (живій пачці emit допише її markdown).
      completedPrefix = parts.join(PAGE_SEP);
      const res = await cleanBatch(batch, emitFor);
    ...
    if (!apiKey) {
      ...
      const r = await polishOne(flatDraft, estimateTokens(flatDraft), emitFor);
      aiCalledAny = true;
