...
//   'full' — поточна поведінка (Document AI повний OCR + артефакти у 02_ОБРОБЛЕНІ);
//   'none' — «без OCR» (TASK 4 D): Claude Vision читає перші 1-2 сторінки →
//            пропонує метадані; файл лише в 01_ОРИГІНАЛИ, артефактів у 02 НЕ
//            створюється. До НАРІЗКИ (slice) НЕ застосовується — там OCR
//            обов'язковий для меж; ocrMode значимий лише у шляху add_as_is.
export const OCR_MODE = Object.freeze({ FULL: 'full', NONE: 'none' });

// Дефолт режиму OCR на додаванні. 'full' = поточна поведінка (Document AI
// повний OCR + артефакти у 02_ОБРОБЛЕНІ). Режим «без OCR» вмикається явно
// тумблером (модалка / DP) — тут лише дефолт, щоб ingest не нав'язував поведінки.
