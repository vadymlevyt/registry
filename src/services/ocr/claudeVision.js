...
import { METADATA_PROMPT, parseMetadataJson } from './visionMetadataParse.js';
...
// renderFileToImages — спільний крок «байти → base64-зображення сторінок».
// Перенесено зі старого тіла extract (load + render) щоб ПЕРЕЮЗАТИ і у повному
// OCR (extract, без ліміту), і у режимі «без OCR» (extractMetadata, maxPages=2).
//   options.startPage (1-based) — пропустити перші startPage-1 сторінок.
//   maxPages (>0) — обмежити кількість РЕНДЕРОВАНИХ сторінок (від startPage).
//                   Infinity (default) — усі сторінки (поведінка повного OCR).
// Повертає { images:[base64], mediaType, pageCount, warnings }.
async function renderFileToImages(file, options = {}, maxPages = Infinity) {
  ...
  if (isPdf) {
    ...
    if (pageCount > LARGE_PDF_THRESHOLD && !Number.isFinite(maxPages)) {
    ...
    // lastPage — обмеження maxPages (режим «без OCR»: лише перші 1-2 сторінки).
    const lastPage = Number.isFinite(maxPages)
      ? Math.min(pageCount, startPage + maxPages - 1)
      : pageCount;
  ...
  return { images, mediaType, pageCount, warnings };
}
...
export default {
  ...
  async extract(file, options = {}) {
    ...
    // 1. Завантажити + відрендерити сторінки (повний OCR — без ліміту maxPages).
    const { images, mediaType, pageCount, warnings } = await renderFileToImages(file, options);
  ...
  // extractMetadata — режим «без OCR» (TASK 4 етап D). Vision читає ПЕРШІ 1-2
  // сторінки → пропонує метадані документа БЕЗ повного OCR і БЕЗ артефактів у 02.
  // Переюзує renderFileToImages (maxPages=2) + ту саму інфру виклику/білінгу що
  // extract (НЕ заводимо ще один прямий fetch — борг #55 не роздуваємо).
  // Повертає { date, category, author, name, gist, pageCount, warnings } — усі
  // поля ПРОПОЗИЦІЇ (адвокат править). Модель — metadataExtractor (Haiku 4.5).
  async extractMetadata(file, options = {}) {
    const apiKey = options.apiKey || (typeof localStorage !== 'undefined' ? localStorage.getItem('claude_api_key') : null);
    if (!apiKey) throw makeError('AUTH', 'Немає API ключа Claude');

    const maxPages = Number(options.maxPages) > 0 ? Number(options.maxPages) : 2;
    const { images, mediaType, pageCount, warnings } = await renderFileToImages(file, options, maxPages);

    const content = images.map((img) => ({
      type: 'image',
      source: { type: 'base64', media_type: mediaType, data: img },
    }));
    content.push({ type: 'text', text: METADATA_PROMPT });

    const model = resolveModel('metadataExtractor');
    let resp;
    try {
      resp = await fetch(ANTHROPIC_ENDPOINT, {
        method: 'POST',
    ...
    const data = await resp.json();
    // Білінг — той самий шов що extract, окремий agentType 'metadata_extractor'
    // (дешевий Vision на 1-2 стор.). НЕ губимо ai_usage+activityTracker (спека D).
    try {
      logAiUsageViaSink({
        agentType: 'metadata_extractor',
        model,
        inputTokens: data?.usage?.input_tokens,
        outputTokens: data?.usage?.output_tokens,
        context: {
      ...
      activityTracker.report('agent_call', {
        ...
        module: MODULES.DOCUMENT_PROCESSOR,
        category: categoryForCase(options.caseId),
        metadata: { agentType: 'metadata_extractor', operation: 'extract_metadata', kind: 'metadata_vision' }
      });
    } catch {}

    const meta = parseMetadataJson(data?.content?.[0]?.text || '');
    return { ...meta, pageCount, warnings };
  },
};
