...
describe('pageMarkers.buildCompactTriagePassport (ФД-0)', () => {
  ...
  describe('абзацна евристика (продовження-абзацу / можливий-абзацний-розрив) ВИДАЛЕНА', () => {
    // Філософія Triage переосмислена: послідовність сторінок — дорадчий
    // орієнтир, не догма (image_merge / fragment_reconstruct прямо
    // передбачають що сторінки одного документа можуть бути розкидані).
    // Стиковка «попередня без крапки + наступна з малої» була СИЛЬНИМ-АНТИ
    // сигналом, який блокував модель від коректної межі за змістом.
    // Прибрано разом із семантичною евристикою; замість неї — інструкція
    // моделі «думай у сукупності, орієнтири — не догма».
    it('сигнал "продовження-абзацу" більше не публікується', () => {
      const out = buildCompactTriagePassport({ schemaVersion: 1, pages: [
        { _text: 'абзац документа продовжується далі без крапки' },
        { _text: 'продовження тексту того ж самого документа' },
      ] });
      expect(out).not.toContain('продовження-абзацу');
    });

    it('сигнал "можливий-абзацний-розрив" більше не публікується', () => {
      const out = buildCompactTriagePassport({ schemaVersion: 1, pages: [
        { _text: 'попередня сторінка закінчена цією фразою.' },
        { _text: 'Новий абзац починається з великої літери після крапки.' },
      ] });
      expect(out).not.toContain('можливий-абзацний-розрив');
    });
  });
